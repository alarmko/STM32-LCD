/*65536 PAGE var ve her PAGE 256 byte
 *4096 sector var 1 sector(4kb)=16*256  byte
 * 256 block var 1 block(32kb)=128*256 byte
 * 1 group(64kb)=256*256 byte
 */
...
/*
#define lcd_buf_size (320*480)
extern uint8_t pBuffer[lcd_buf_size];
asm(".equ pBuffer, 0xD0000000");
*/
w25qxx_t	w25qxx;
...
void	W25qxx_EraseChip(void)
{
	...
	W25qxx_WriteEnable();
	HAL_GPIO_WritePin(W25Q_CS_GPIO_Port,W25Q_CS_Pin,GPIO_PIN_RESET);
  W25qxx_Spi(0x60);  //erase chip  C7h VEYA 60h OLACAK
  HAL_GPIO_WritePin(W25Q_CS_GPIO_Port,W25Q_CS_Pin,GPIO_PIN_SET);
...
void W25qxx_EraseBlock(uint32_t BlockAddr)
{
  ...
  W25qxx_WriteEnable();
  HAL_GPIO_WritePin(W25Q_CS_GPIO_Port,W25Q_CS_Pin,GPIO_PIN_RESET);
  W25qxx_Spi(0xD8);  //erase block 64kbyte
	if(w25qxx.ID>=W25Q256)
...
//###################################################################################################################
void W25qxx_ReadBytes_BULENT(uint8_t* pBuffer, uint32_t ReadAddr, uint32_t NumByteToRead)
{
/*
	MX_SPI1_Init();
	while(w25qxx.Lock==1)
...
//	  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_SET);
	...
	HAL_Delay(100);
	  ...
	  GPIO_InitStruct1.Pull = GPIO_NOPULL;
	  GPIO_InitStruct1.Speed = GPIO_SPEED_FREQ_HIGH;
	  GPIO_InitStruct1.Alternate=GPIO_AF0_SPI1;
	  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct1);
